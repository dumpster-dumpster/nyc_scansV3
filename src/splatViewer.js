...
function packHalf2x16(x, y) {
    ...
    function processPlyBuffer(inputBuffer, crop) {
...
export async function initSplatViewer(containerId, splatFile) {
    ...
    if (!stopLoading) {            // Remove the problematic crop bounds that filter everything out
            const crop = {
                minX: -3, maxX: 3,   // Expanded from 0,0 to reasonable bounds
                minY: -3, maxY: 3,   // Expanded from 0,0 to reasonable bounds
                minZ: -3, maxZ: 3,   // Expanded from 0,0 to reasonable bounds
            };
            
            worker.postMessage({ ply: splatData.buffer.slice(0, bytesRead), save: false, crop });
        } else {
